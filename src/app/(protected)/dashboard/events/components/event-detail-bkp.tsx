...
const EventDetailsModal = ({
  ...
  // Evento visualizado após mesclar alterações locais ainda não persistidas
  const viewEvent = {
    ...(event as Event),
    ...(editedEvent as Partial<Event>),
  } as Event;
  const isDraft = viewEvent.status === "draft";
  ...
  const handleSaveEvent = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        if (result.data) {
          // Atualizar o estado local para refletir exatamente o que o servidor persistiu
          setEditedEvent((prev) => ({ ...prev, ...(result.data as any) }));
        }
        // Não fechar modal nem fazer refresh; apenas sair do modo de edição
        setIsEditing(false);
      } else {
        showToast({
          type: "error",
  ...
  const handleEditName = () => {
    if (!event) return;
    setEditedName((editedEvent.name as string) || event.name);
    setIsEditingName(true);
  };

  // Aplicar nome apenas localmente; persistir em handleSaveEvent
  const handleSaveName = async () => {
    if (!event || !editedName.trim()) return;
    setEditedEvent((prev) => ({ ...prev, name: editedName.trim() }));
    setIsEditingName(false);
  };
      ...
      <DropdownMenu open={showMenu} onOpenChange={setShowMenu}>
        ...
        <DropdownMenuContent>
          ...
          <DropdownMenuItem
            ...
            className="flex items-center"
            disabled={viewEvent?.status !== "draft"}
          >
  ...
  return (
    <>
      <Modal
        ...
        subtitle={(editedEvent.name as string) || event.name}
          ...
          <div className="space-y-8">
              ...
              {isEditingName ? (
                ...
                <div className="flex items-center gap-2 group">
                  <h2 className="text-xl font-semibold text-cinza-chumbo">
                    {(editedEvent.name as string) || event.name}
                  </h2>
                  <button
              ...
              <div>
                ...
                {isEditing ? (
                  ...
                  <p className="font-semibold text-cinza-chumbo">
                    {EVENT_TYPES.find((type) => type.value === viewEvent.type)
                      ?.label || getTypeText(viewEvent.type)}
                  </p>
                )}
                ...
                {isEditing ? (
                  ...
                  <p className="font-semibold text-cinza-chumbo">
                    {EVENT_CATEGORIES.find(
                      (cat) => cat.value === viewEvent.category
                    )?.label || getCategoryText(viewEvent.category)}
                  </p>
                )}
                {isEditing && event.status !== "draft" && (
                  <p className="text-xs text-cinza-chumbo/60 mt-1">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  ...
                  {isEditing ? (
                    ...
                    <>
                      <p className="font-semibold text-cinza-chumbo">
                        {formatEventDate(viewEvent.startDate)}
                      </p>
                      <p className="text-sm text-cinza-chumbo/70">
                        às {formatEventTime(viewEvent.startDate)}
                      </p>
                    </>
                  )}
                </div>
                ...
                <div>
                  ...
                  {isEditing ? (
                    ...
                    <>
                      <p className="font-semibold text-cinza-chumbo">
                        {formatEventDate(viewEvent.endDate as Date)}
                      </p>
                      <p className="text-sm text-cinza-chumbo/70">
                        às {formatEventTime(viewEvent.endDate as Date)}
                      </p>
                    </>
                  ) : (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  ...
                  {isEditing ? (
                    ...
                    <>
                      <p className="font-semibold text-cinza-chumbo">
                        {formatEventDate(viewEvent.registrationStartDate)}
                      </p>
                      <p className="text-sm text-cinza-chumbo/70">
                        às {formatEventTime(viewEvent.registrationStartDate)}
                      </p>
                    </>
                  ) : (
                    <p className="text-sm text-cinza-chumbo/50 italic">
                ...
                <div>
                  ...
                  {isEditing ? (
                    ...
                    <>
                      <p className="font-semibold text-cinza-chumbo">
                        {formatEventDate(viewEvent.registrationEndDate)}
                      </p>
                      <p className="text-sm text-cinza-chumbo/70">
                        às {formatEventTime(viewEvent.registrationEndDate)}
                      </p>
                    </>
                  ) : (
                    <p className="text-sm text-cinza-chumbo/50 italic">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  ...
                  {isEditing ? (
                    ...
                    <>
                      <p className="font-semibold text-cinza-chumbo">
                        {viewEvent.isPublic ? "Público" : "Privado"}
                      </p>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-3">
                ...
                {["published", "ongoing"].includes(viewEvent.status) && (
                  <button
